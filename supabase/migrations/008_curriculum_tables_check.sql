...
DO $$
BEGIN
    -- textbook_routesテーブルが存在しない場合のみ作成
    IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'textbook_routes') THEN
    ...
    -- student_levelsテーブルが存在しない場合のみ作成
    IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'student_levels') THEN
    ...
    -- study_plansテーブルが存在しない場合のみ作成
    IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'study_plans') THEN
    ...
    -- study_tasksテーブルが存在しない場合のみ作成
    IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'study_tasks') THEN
...
DROP POLICY IF EXISTS "組織メンバーは自分の組織の参考書ルートを表示可能" ON textbook_routes;
...
DROP POLICY IF EXISTS "講師以上の権限で参考書ルートを作成可能" ON textbook_routes;
...
DROP POLICY IF EXISTS "講師以上の権限で参考書ルートを更新可能" ON textbook_routes;
...
DROP POLICY IF EXISTS "講師以上の権限で参考書ルートを削除可能" ON textbook_routes;
...
-- textbooks ポリシー
DROP POLICY IF EXISTS "組織メンバーは参考書を表示可能" ON textbooks;
...
DROP POLICY IF EXISTS "講師以上の権限で参考書を作成可能" ON textbooks;
...
DROP POLICY IF EXISTS "講師以上の権限で参考書を更新可能" ON textbooks;
...
DROP POLICY IF EXISTS "講師以上の権限で参考書を削除可能" ON textbooks;
...
-- student_levels ポリシー
DROP POLICY IF EXISTS "生徒は自分のレベルを表示可能" ON student_levels;
...
DROP POLICY IF EXISTS "講師以上の権限で生徒レベルを作成可能" ON student_levels;
...
DROP POLICY IF EXISTS "講師以上の権限で生徒レベルを更新可能" ON student_levels;
...
-- study_plans ポリシー
DROP POLICY IF EXISTS "組織メンバーは学習計画を表示可能" ON study_plans;
...
DROP POLICY IF EXISTS "講師以上の権限で学習計画を作成可能" ON study_plans;
...
DROP POLICY IF EXISTS "講師以上の権限で学習計画を更新可能" ON study_plans;
...
DROP POLICY IF EXISTS "講師以上の権限で学習計画を削除可能" ON study_plans;
...
-- study_tasks ポリシー
DROP POLICY IF EXISTS "組織メンバーは学習タスクを表示可能" ON study_tasks;
...
DROP POLICY IF EXISTS "講師以上の権限で学習タスクを作成可能" ON study_tasks;
...
DROP POLICY IF EXISTS "講師以上の権限で学習タスクを更新可能" ON study_tasks;
...
DROP POLICY IF EXISTS "講師以上の権限で学習タスクを削除可能" ON study_tasks;
...
-- インデックスが存在しない場合のみ作成
CREATE INDEX IF NOT EXISTS idx_textbook_routes_organization ON textbook_routes(organization_id);
